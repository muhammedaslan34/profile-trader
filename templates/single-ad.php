...
// Get linked trader ID - check our meta key first, fallback to trader_link
...
if (!$trader_id) {
    ...
    } elseif (is_string($trader_link) && filter_var($trader_link, FILTER_VALIDATE_URL)) {
        // Handle URL values (from JetEngine) - convert to post ID
        $trader_id = url_to_postid($trader_link);
    }
}

// If no trader ID found, try to find trader by matching advertiser name with trader post title
if (!$trader_id && !empty($advertisers_name)) {
    $search_name = trim($advertisers_name);

    // Try exact match first using WP_Query (replaces deprecated get_page_by_title)
    $trader_query = new WP_Query([
        'post_type' => 'trader',
        'post_status' => 'publish',
        'title' => $search_name,
        'posts_per_page' => 1
    ...
    if ($trader_query->have_posts()) {
        $trader_id = $trader_query->posts[0]->ID;
    } else {
        // Fallback: flexible search (partial match)
        global $wpdb;
        $trader_id = $wpdb->get_var($wpdb->prepare(
            "SELECT ID FROM {$wpdb->posts}
             WHERE post_type = 'trader'
    ...
    // Save the relationship for future use
    if ($trader_id) {
        update_post_meta($ad_id, '_pt_trader_id', $trader_id);
    }
...
if ($trader_id) {
    ...
    if ($trader_post && $trader_post->post_type === 'trader') {
        $trader_logo_id = get_post_meta($trader_id, 'logo', true);
         ...
         // Use direct trader permalink (e.g., /trader/trader-name/)
         $trader_profile_url = get_permalink($trader_id);
...
// If we have advertiser name but no trader_info, try to create a basic profile link
if (!$trader_info && !empty($advertisers_name)) {
    global $wpdb;
    $search_name = trim($advertisers_name);
    $trader_id_found = $wpdb->get_var($wpdb->prepare(
        "SELECT ID FROM {$wpdb->posts}
         WHERE post_type = 'trader'
         AND post_status = 'publish'
         AND post_title LIKE %s
    ...
    if ($trader_id_found) {
        $trader_post = get_post($trader_id_found);
        if ($trader_post) {
            $trader_id = $trader_id_found;
            update_post_meta($ad_id, '_pt_trader_id', $trader_id);

            $trader_logo_id = get_post_meta($trader_id, 'logo', true);
            $profile_page = get_page_by_path('trader-profile');
            $trader_profile_url = $profile_page
                ? add_query_arg('trader_id', $trader_id, get_permalink($profile_page->ID))
                : add_query_arg('trader_id', $trader_id, home_url());
            ...
            <section class="pt-ad-section pt-ad-specs">
                ...
                <div class="pt-specs-grid">
                    ...
                    <div class="pt-spec-item">
                        ...
                        <div>
                            <p class="pt-spec-label">المشاهدات</p>
                            <p class="pt-spec-value"><?php echo number_format_i18n(PT_Ad_Views::get_instance()->get_total_views($ad_id)); ?></p>
                        </div>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="pt-ad-sidebar-sticky">
                ...
                <div class="pt-ad-price-card">
                    ...
                    <div class="pt-ad-actions">
                        <?php if ($trader_info && !empty($trader_info['url'])): ?>
                        <a href="<?php echo esc_url($trader_info['url']); ?>" class="pt-btn pt-btn-profile pt-btn-full">
                ...
                <div class="pt-ad-seller-card">
                    <div class="pt-seller-header">
                        <h3>عن المعلن</h3>
                        <?php 
                        // SIMPLIFIED: Always generate URL when trader is connected
                        $profile_url = null;
                        $final_trader_id = 0;
                        
                        // Step 1: Get trader ID - prioritize trader_id from meta (when connected in admin)
                        // THIS IS THE MAIN CASE - when trader is linked in admin
                        if ($trader_id && $trader_id > 0) {
                            $final_trader_id = $trader_id;
                        } elseif ($trader_info && !empty($trader_info['url'])) {
                            // Use trader_info if available
                            $profile_url = $trader_info['url'];
                            $final_trader_id = $trader_id;
                        } elseif (!empty($advertisers_name)) {
                            // Try to find trader by name
                            global $wpdb;
                            $search_name = trim($advertisers_name);
                            ...
                            if ($found_id) {
                                $final_trader_id = $found_id;
                                update_post_meta($ad_id, '_pt_trader_id', $found_id);
                            }
                        }
                        
                        // Step 2: Generate URL if we have trader_id but no URL yet
                        if ($final_trader_id && $final_trader_id > 0 && !$profile_url) {
                            // Find profile page by slug first (most reliable)
                            $profile_page = get_page_by_path('trader-profile');
                            if ($profile_page) {
                                $profile_url = add_query_arg('trader_id', $final_trader_id, get_permalink($profile_page->ID));
                            } else {
                                // Fallback to home URL
                                $profile_url = add_query_arg('trader_id', $final_trader_id, home_url());
                            }
                        }
                        
                        // Final fallback - use trader_id if final_trader_id not set
                        if (!$profile_url && $trader_id && $trader_id > 0) {
                            $profile_page = get_page_by_path('trader-profile');
                            if ($profile_page) {
                                $profile_url = add_query_arg('trader_id', $trader_id, get_permalink($profile_page->ID));
                            }
                        }

                        // Last resort - use # if still no URL
                        if (!$profile_url) {
                            $profile_url = '#';
                        }
                        ...
                        <a href="<?php echo esc_url($profile_url); ?>" 
                           class="pt-view-profile" 
                           <?php if ($final_trader_id): ?>data-trader-id="<?php echo esc_attr($final_trader_id); ?>"<?php endif; ?>>
                            عرض الملف
                        </a>
                    </div>
...
<?php
// Track view for this ad
if (class_exists('PT_Ad_Views') && $ad_id) {
    $view_nonce = wp_create_nonce('pt_view_nonce');
    ?>
    <script>
    (function() {
        var tracked = false;
        function ptTrackAdView() {
            if (tracked) return;
            tracked = true;
            console.log('PT Views: Tracking ad view for ID <?php echo $ad_id; ?>');
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4) {
                    console.log('PT Views Response:', xhr.status, xhr.responseText);
                }
            };
            xhr.open('POST', '<?php echo admin_url('admin-ajax.php'); ?>', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.send('action=pt_track_ad_view&post_id=<?php echo $ad_id; ?>&post_type=ads&nonce=<?php echo $view_nonce; ?>');
        }
        setTimeout(ptTrackAdView, 1000);
    })();
    </script>
    <?php
}
