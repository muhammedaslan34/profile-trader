...
// Query all ads for this trader using _pt_trader_id (primary) and fallback keys
global $wpdb;

// Primary: Get ads linked via _pt_trader_id (our custom meta key)
$ad_ids = $wpdb->get_col($wpdb->prepare(
    "SELECT DISTINCT p.ID
     ...
     AND pm.meta_key = '_pt_trader_id'
     AND pm.meta_value = %s
     ORDER BY p.post_date DESC",
    $trader_id
));

// Fallback: Also check trader_link for legacy ads (numeric values only)
$legacy_ids = $wpdb->get_col($wpdb->prepare(
    "SELECT DISTINCT p.ID
     ...
     WHERE p.post_type = 'ads'
     AND p.post_status = 'publish'
     AND pm.meta_key IN ('trader_link', '_trader_link')
     AND pm.meta_value = %s
     ORDER BY p.post_date DESC",
    $trader_id
));

$ad_ids = array_unique(array_merge($ad_ids, $legacy_ids));

// Create WP_Query with the found IDs
// Only show published ads (future posts will show when they're published)
if (!empty($ad_ids)) {
    ...
    <div class="pt-trader-header">
        ...
        <div class="pt-trader-info">
            ...
            <div class="pt-trader-badges">
                ...
                <div class="pt-trader-badge pt-views-badge">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                        <circle cx="12" cy="12" r="3"></circle>
                    </svg>
                    <span><?php echo number_format_i18n(PT_Ad_Views::get_instance()->get_total_views($trader_id)); ?> مشاهدة</span>
                </div>
                <?php endif; ?>
            </div>
...
.pt-avatar-placeholder-large {
    ...
    background: linear-gradient(135deg, var(--pt-accent) 0%, var(--pt-accent-dark) 100%);
...
.pt-trader-badge.pt-views-badge {
...
.pt-section-header .pt-section-title {
    margin-bottom: 0;
}
...
if (class_exists('PT_Ad_Views') && $trader_id) {
    ...
    (function() {
        ...
        function ptTrackTraderView() {
            ...
            xhr.send('action=pt_track_ad_view&post_id=<?php echo $trader_id; ?>&post_type=trader&nonce=<?php echo $view_nonce; ?>');
        }
        setTimeout(ptTrackTraderView, 1000);
    })();
    </script>
    <?php
}
